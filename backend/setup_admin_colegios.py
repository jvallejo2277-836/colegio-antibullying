...
from core.models import CustomUser, Colegio, ColegioAsignado
...
def configurar_admin_colegios():
    ...
    try:
        ...
        print("❌ Admin no existe! Créalo primero con: python manage.py createsuperuser")
        return
    ...
    for i, colegio in enumerate(colegios):
        ...
        print(f"   {'✅ Creado' if created else '⚠️  Ya existe'}: {colegio.nombre} {'(PRINCIPAL)' if i == 0 else ''}")
    ...
    print(f"   • Colegios asignados: {ColegioAsignado.objects.filter(usuario=admin).count()}")
    ...
    for asignacion in asignaciones:
        print(f"   • {asignacion.colegio.nombre} {'(PRINCIPAL)' if asignacion.es_colegio_principal else ''}")
