...
from core.models import CustomUser, PerfilUsuario, Colegio
...
for user in users:
    ...
    print(f"{user.username:<20} {user.role:<20} {str(user.colegio_id):<12} {colegio_name}")
...
for perfil in perfiles:
    print(f"{perfil.user.id:<8} {perfil.user.username:<20} {perfil.tipo_usuario:<20} {perfil.colegio.id}")
...
users_con_multiples_perfiles = PerfilUsuario.objects.values('user').annotate(count=Count('user')).filter(count__gt=1)
...
for perfil in perfiles:
    if perfil.user.colegio_id != perfil.colegio_id:
        print(f"   - {perfil.user.username}: CustomUser.colegio_id={perfil.user.colegio_id}, PerfilUsuario.colegio_id={perfil.colegio_id}")