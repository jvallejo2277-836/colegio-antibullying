...
class GraphsView(APIView):
    ...
    def get(self, request, format=None):
        ...
        # Reportes por estado
        reportes_por_estado = (
            IncidentReport.objects.values("estado")
        ...
        reportes_por_gravedad = (
            IncidentReport.objects.values("tipo_incidente__gravedad")
            .annotate(count=Count("id"))
            .order_by("tipo_incidente__gravedad")
        )
        ...
        reportes_criticos = IncidentReport.objects.filter(
            Q(urgencia="critica")
            | Q(
                fecha_limite_investigacion__lt=now,
...
class TipoIncidenteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(activo=True).order_by(
            "es_categoria_legal", "categoria", "nombre"
        )

    def perform_create(self, serializer):
        """Personalizar creación de tipos de incidente"""
        # Si no se especifica colegio, usar el del usuario actual (si no es admin)
        if not serializer.validated_data.get("colegio") and hasattr(
            self.request.user, "colegio"
        ):
        ...
        if not instance.puede_eliminar:
            ...
            if instance.es_categoria_legal:
                raise ValidationError(
                    "No se pueden eliminar las categorías definidas por ley"
                )
            else:
                raise ValidationError(
                    "No se puede eliminar un tipo que tiene incidentes reportados"
                )
    ...
    def por_colegio(self, request):
        ...
        tipos_legales = TipoIncidente.objects.filter(
            es_categoria_legal=True, colegio__isnull=True, activo=True
        )

        # 2. Tipos específicos del colegio
        tipos_colegio = TipoIncidente.objects.filter(colegio_id=colegio_id, activo=True)
...
class IncidentReportViewSet(viewsets.ModelViewSet):
    queryset = (
        IncidentReport.objects.select_related("colegio", "tipo_incidente", "reportero")
        .prefetch_related("estudiantes_involucrados", "evidencias")
        .order_by("-created_at")
    )
...
class ProtocoloProcesoViewSet(viewsets.ModelViewSet):
    ...
    queryset = ProtocoloProceso.objects.select_related("colegio").prefetch_related(
        "etapas"
    )
...
class ProcesoIncidenteViewSet(viewsets.ModelViewSet):
    ...
    def avanzar_etapa(self, request, pk=None):
        ...
        siguiente_etapa = EtapaProtocolo.objects.filter(
            protocolo=proceso.protocolo, orden__gt=proceso.etapa_actual.orden
        ).first()
    ...
    def completar(self, request, pk=None):
        ...
        if proceso.etapa_actual == ejecucion.etapa:
            ...
            siguiente = EtapaProtocolo.objects.filter(
                protocolo=proceso.protocolo, orden__gt=ejecucion.etapa.orden
            ).first()
...
class AccesoIdentidadViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ["incidente", "usuario_autorizado", "tipo_acceso", "activo"]
