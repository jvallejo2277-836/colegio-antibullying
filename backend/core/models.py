...
class CustomUser(AbstractUser):
    ...
    role = models.CharField(
        max_length=25, choices=ROLE_CHOICES, default="encargado_convivencia"
    )
    ...
    def get_colegios_asignados(self):
        ...
        return (
            ColegioAsignado.objects.filter(usuario=self, activo=True)
            .select_related("colegio")
            .order_by("-es_colegio_principal")
        )
    ...
    def get_colegio_principal(self):
        ...
        try:
            asignacion = ColegioAsignado.objects.get(
                usuario=self, es_colegio_principal=True, activo=True
            )
    ...
    def asignar_colegio(self, colegio, es_principal=False):
        ...
        if self.role != "admin":
            raise ValueError(
                "Solo los administradores pueden tener múltiples colegios asignados"
            )
    ...
    def cambiar_colegio_activo(self, colegio_id):
        ...
        try:
            asignacion = ColegioAsignado.objects.get(
                usuario=self, colegio_id=colegio_id, activo=True
            )
...
class ColegioAsignado(models.Model):
    ...
    es_colegio_principal = models.BooleanField(
        default=False, help_text="Indica si es el colegio principal del usuario"
    )
    ...
    def clean(self):
        ...
        if (
            ...
            and ColegioAsignado.objects.filter(
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["usuario", "colegio"], name="unique_usuario_colegio"
            ),
            models.UniqueConstraint(
                fields=["usuario"],
        ...
        indexes = [
            ...
            models.Index(
                fields=["usuario", "es_colegio_principal"],
                name="core_col_principal_idx",
            ),
        ]
...
class PerfilUsuario(models.Model):
    ...
    numero_registro_profesional = models.CharField(
        max_length=50, blank=True, help_text="Número de registro en colegio profesional"
    )
    jornada_laboral_horas = models.PositiveIntegerField(
        null=True,
        blank=True,
    ...
    certificacion_convivencia_escolar = models.BooleanField(
        default=False, help_text="¿Tiene certificación en convivencia escolar?"
    )
    fecha_ultima_capacitacion = models.DateField(null=True, blank=True)
    instituciones_capacitacion = models.TextField(
    ...
    def puede_resolver_casos_graves(self):
        ...
        return (
            self.es_encargado_convivencia
            and self.cumple_requisitos_encargado
            and self.certificacion_convivencia_escolar
        )
    ...
    def get_casos_asignados_pendientes(self):
        ...
        return IncidentReport.objects.filter(
            asignado_a=self, estado__in=["recibido", "en_investigacion"]
...
class IncidentReport(models.Model):
    ...
    testigos = models.ManyToManyField(
        PerfilUsuario, related_name="incidentes_testigo", blank=True
    )
    ...
    reportero_externo_nombre = models.CharField(
        max_length=255, blank=True, help_text="Si el reportero no está en el sistema"
    )
    ...
    def verificar_denuncia_obligatoria(self):
        ...
        if (
            self.tipo_incidente.categoria in delitos_que_requieren_denuncia
            or self.tipo_incidente.requiere_denuncia
        ):
            ...
            if not denunciante or denunciante.tipo_usuario not in [
                "directivo",
                "encargado_convivencia",
            ]:
    ...
    def get_encargado_convivencia(self):
        ...
        try:
            return PerfilUsuario.objects.get(
                colegio=self.colegio, tipo_usuario="encargado_convivencia"
            )
    ...
    def get_directivo_colegio(self):
        ...
        try:
            return PerfilUsuario.objects.filter(
                colegio=self.colegio, tipo_usuario="directivo"
            ).first()
        except PerfilUsuario.DoesNotExist:
            return None
    ...
    def es_acoso_escolar_segun_ley(self):
        ...
        if any(
            palabra in self.descripcion.lower()
            for palabra in palabras_clave_superioridad
        ):
            criterios_cumplidos += 1
        ...
        if hasattr(self, "medidas_disciplinarias"):
            ...
            if medidas.exists():
                cumplimiento["debido_proceso"] = all(
                    m.fecha_notificacion_apoderado is not None for m in medidas
                )
    ...
    def iniciar_proceso_automatico(self):
        ...
        try:
            ...
            protocolo = ProtocoloProceso.objects.get(
                colegio=self.colegio, gravedad=self.tipo_incidente.gravedad, activo=True
            )

            # Crear proceso
            proceso = ProcesoIncidente.objects.create(
                incidente=self, protocolo=protocolo
            )
    ...
    def puede_ver_identidad_denunciante(self, usuario_solicitante):
        ...
        try:
            regla = ReglasAnonimato.objects.get(
                colegio=self.colegio, nivel_anonimato=self.nivel_anonimato
            )

            # Verificar si su rol tiene acceso automático
            roles_permitidos = [r.strip() for r in regla.roles_con_acceso.split(",")]
        ...
        return AccesoIdentidadDenunciante.objects.filter(
            incidente=self, usuario_autorizado=usuario_solicitante, activo=True
        ).exists()
...
def evidencia_upload_path(instance, filename):
    ...
    fecha = timezone.now()
    return os.path.join(
        "evidencias", str(fecha.year), f"{fecha.month:02d}", nuevo_nombre
    )
...
class Evidence(models.Model):
    ...
    def __str__(self):
        titulo = (
            self.reporte.titulo[:30] + "..."
            if len(self.reporte.titulo) > 30
            else self.reporte.titulo
        )
...
class ReglamentoInterno(models.Model):
    ...
    colegio = models.OneToOneField(
        Colegio, on_delete=models.CASCADE, related_name="reglamento_interno"
    )
    ...
    archivo_reglamento = models.FileField(
        upload_to="reglamentos/", null=True, blank=True
    )
...
class DenunciaObligatoria(models.Model):
    ...
    incidente = models.OneToOneField(
        IncidentReport, on_delete=models.CASCADE, related_name="denuncia_obligatoria"
    )
    ...
    autoridad_denunciada = models.CharField(
        max_length=20, choices=AUTORIDADES_CHOICES, blank=True
    )
    numero_denuncia = models.CharField(max_length=100, blank=True)

    # Responsable de la denuncia
    denunciante_institucional = models.ForeignKey(
        PerfilUsuario,
        on_delete=models.PROTECT,
        null=True,
        blank=True,
        limit_choices_to={"tipo_usuario__in": ["directivo", "encargado_convivencia"]},
    )

    # Documentación
    documento_denuncia = models.FileField(upload_to="denuncias/", null=True, blank=True)
    ...
    def esta_vencida(self):
        ...
        return (
            timezone.now() > self.fecha_limite_denuncia
            and not self.fecha_denuncia_realizada
        )
...
class MedidaDisciplinaria(models.Model):
    ...
    incidente = models.ForeignKey(
        IncidentReport, on_delete=models.CASCADE, related_name="medidas_disciplinarias"
    )
    ...
    gravedad_infraccion = models.CharField(
        max_length=15, choices=GRAVEDAD_INFRACCION_CHOICES
    )
    ...
    def save(self, *args, **kwargs):
        if not self.fecha_limite_apelacion and self.fecha_notificacion_estudiante:
            self.fecha_limite_apelacion = (
                self.fecha_notificacion_estudiante
...
class AlertaLegal(models.Model):
    ...
    incidente = models.ForeignKey(
        IncidentReport, on_delete=models.CASCADE, related_name="alertas_legales"
    )
    ...
    estado = models.CharField(
        max_length=15, choices=ESTADO_ALERTA_CHOICES, default="activa"
    )

    # Notificación
    destinatarios = models.ManyToManyField(
        PerfilUsuario, related_name="alertas_recibidas"
    )
...
class CumplimientoLey20536(models.Model):
    ...
    colegio = models.ForeignKey(
        Colegio, on_delete=models.CASCADE, related_name="cumplimiento_ley"
    )
    ...
    porcentaje_denuncias_plazo = models.DecimalField(
        max_digits=5, decimal_places=2, default=0
    )
    porcentaje_notificaciones_plazo = models.DecimalField(
        max_digits=5, decimal_places=2, default=0
    )
    porcentaje_resoluciones_plazo = models.DecimalField(
        max_digits=5, decimal_places=2, default=0
    )
    porcentaje_debido_proceso = models.DecimalField(
        max_digits=5, decimal_places=2, default=0
    )

    # Cumplimiento general
    cumplimiento_general = models.DecimalField(
        max_digits=5, decimal_places=2, default=0
    )
    ...
    def calcular_cumplimiento(self):
        ...
        if self.total_casos > 0:
            ...
            self.casos_requieren_denuncia = casos.filter(
                tipo_incidente__requiere_denuncia=True
            ).count()

            if self.casos_requieren_denuncia > 0:
                denuncias_plazo = (
                    DenunciaObligatoria.objects.filter(
                        incidente__in=casos, fecha_denuncia_realizada__isnull=False
                    )
                    .filter(
                ...
                self.porcentaje_denuncias_plazo = (
                    denuncias_plazo / self.casos_requieren_denuncia
                ) * 100
...
class NotificacionLegal(models.Model):
    ...
    incidente = models.ForeignKey(
        IncidentReport, on_delete=models.CASCADE, related_name="notificaciones_legales"
    )
    tipo_notificacion = models.CharField(
        max_length=30, choices=TIPO_NOTIFICACION_CHOICES
    )
    ...
    numero_seguimiento = models.CharField(
        max_length=100, blank=True, help_text="Número de seguimiento oficial"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.fecha_limite:
            self.fecha_limite = self.incidente.created_at + timedelta(
                hours=self.plazo_maximo_horas
            )
        super().save(*args, **kwargs)
    ...
    def esta_vencida(self):
        ...
        return self.estado == "pendiente" and timezone.now() > self.fecha_limite
...
class PlazoLegal(models.Model):
    ...
    incidente = models.ForeignKey(
        IncidentReport, on_delete=models.CASCADE, related_name="plazos_legales"
    )
    ...
    estado = models.CharField(
        max_length=15, choices=ESTADO_CUMPLIMIENTO_CHOICES, default="pendiente"
    )
    ...
    def esta_vencido(self):
        ...
        return (
            self.estado in ["pendiente", "en_curso"]
...
class SeguimientoLegal(models.Model):
    ...
    incidente = models.OneToOneField(
        IncidentReport, on_delete=models.CASCADE, related_name="seguimiento_legal"
    )
    ...
    clasificacion_legal = models.CharField(
        max_length=20, choices=CLASIFICACION_LEGAL_CHOICES
    )
    ...
    # Documentación legal
    documentos_proceso = models.FileField(
        upload_to="procesos_legales/", null=True, blank=True
    )
    actas_generadas = models.TextField(
...
class ReporteMINEDUC(models.Model):
    ...
    colegio = models.ForeignKey(
        Colegio, on_delete=models.CASCADE, related_name="reportes_mineduc"
    )
    ...
    estado = models.CharField(
        max_length=15, choices=ESTADO_REPORTE_CHOICES, default="borrador"
    )

    # Archivos
    archivo_reporte = models.FileField(
        upload_to="reportes_mineduc/", null=True, blank=True
    )
    ...
    def generar_estadisticas(self):
        ...
        self.casos_tipo_i = incidentes.filter(tipo_incidente__gravedad="leve").count()
        self.casos_tipo_ii = incidentes.filter(tipo_incidente__gravedad="grave").count()
        self.casos_tipo_iii = incidentes.filter(
            tipo_incidente__gravedad="muy_grave"
        ).count()
        ...
        resoluciones = ResolucionIncidente.objects.filter(incidente__in=incidentes)
        self.medidas_formativas_aplicadas = sum(
            r.medidas_formativas.count() for r in resoluciones
        )
        self.sanciones_aplicadas = sum(r.sanciones.count() for r in resoluciones)
...
class EtapaProtocolo(models.Model):
    ...
    protocolo = models.ForeignKey(
        ProtocoloProceso, on_delete=models.CASCADE, related_name="etapas"
    )
...
class ProcesoIncidente(models.Model):
    ...
    incidente = models.OneToOneField(
        IncidentReport, on_delete=models.CASCADE, related_name="proceso"
    )
    protocolo = models.ForeignKey(ProtocoloProceso, on_delete=models.PROTECT)

    # Estado del proceso
    etapa_actual = models.ForeignKey(
        EtapaProtocolo, on_delete=models.PROTECT, null=True, blank=True
    )
    ...
    estado = models.CharField(
        max_length=15, choices=ESTADO_PROCESO_CHOICES, default="iniciado"
    )
            ...
            EjecucionEtapa.objects.get_or_create(
                ...
                defaults={
                    "modificada_por_anonimato": (
                        self.incidente.solicita_anonimato
                        and etapa.accion_si_anonimo == "modificada"
                    )
                },
            )
...
class EjecucionEtapa(models.Model):
    ...
    proceso = models.ForeignKey(
        ProcesoIncidente, on_delete=models.CASCADE, related_name="ejecuciones"
    )
    ...
    estado = models.CharField(
        max_length=15, choices=ESTADO_EJECUCION_CHOICES, default="pendiente"
    )
    ...
    observaciones = models.TextField(blank=True)
    archivos_adjuntos = models.FileField(upload_to="etapas/", null=True, blank=True)
    ...
    def __str__(self):
        return (
            f"{self.proceso.incidente.titulo} - "
...
class AccesoIdentidadDenunciante(models.Model):
    ...
    incidente = models.ForeignKey(
        IncidentReport, on_delete=models.CASCADE, related_name="accesos_identidad"
    )
    usuario_autorizado = models.ForeignKey(PerfilUsuario, on_delete=models.CASCADE)
    ...
    autorizado_por = models.ForeignKey(
        PerfilUsuario, on_delete=models.PROTECT, related_name="accesos_autorizados"
    )
    ...
    def __str__(self):
        return f"Acceso de {self.usuario_autorizado} a " f"{self.incidente.titulo}"
