...
class Colegio(models.Model):
    ...
    rbd = models.CharField(
        max_length=20, unique=True, help_text="Rol Base de Datos MINEDUC"
    )
...
class PerfilUsuario(models.Model):
    ...
    def __str__(self):
        return f"{self.user.get_full_name()} - " f"{self.get_tipo_usuario_display()}"
...
class IncidentReport(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.fecha_limite_investigacion and self.tipo_incidente:
            self.fecha_limite_investigacion = self.created_at + timedelta(
...
class Evidence(models.Model):
    ...
    def __str__(self):
        return (
            f"Evidencia {self.get_tipo_evidencia_display()} - " f"{self.reporte.titulo}"
        )
