...
from core.models import ColegioAsignado, CustomUser
...
def mostrar_asignaciones_admin():
    ...
    try:
        ...
        print(
            f"📍 Colegio Actual (colegio_id): {admin_user.colegio_id} - {admin_user.colegio.nombre if admin_user.colegio else 'N/A'}"
        )

        # Obtener asignaciones
        asignaciones = (
            ColegioAsignado.objects.filter(usuario=admin_user, activo=True)
            .select_related("colegio")
            .order_by("-es_colegio_principal", "colegio__nombre")
        )
        ...
        if asignaciones.exists():
            ...
            for asignacion in asignaciones:
                principal = (
                    "🌟 PRINCIPAL" if asignacion.es_colegio_principal else "  Regular"
                )
                print(
                    f"{principal} | ID: {asignacion.colegio.id} | {asignacion.colegio.nombre}"
                )
                print(
                    f"          | Asignado: {asignacion.fecha_asignacion.strftime('%Y-%m-%d %H:%M')}"
                )
                if asignacion.es_colegio_principal:
                    print("          | ⚡ ACTIVO EN SELECTOR")
                print()
        else:
        ...
        print(
            f"🔍 Método get_colegios_permitidos(): {colegios_permitidos.count()} colegios"
        )
