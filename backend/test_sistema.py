...
def test_imports():
    ...
    try:
        from core.models import (
            Colegio, IncidentReport, TipoIncidente, PerfilUsuario,
            MedidaFormativa, Sancion, ResolucionIncidente,
            # Nuevos modelos del sistema de protocolos
            ProtocoloProceso, EtapaProtocolo, ProcesoIncidente,
            EjecucionEtapa, ReglasAnonimato, AccesoIdentidadDenunciante
        )
...
def test_serializers():
    ...
    try:
        from core.serializers import (
            ColegioSerializer, IncidentReportSerializer, TipoIncidenteSerializer,
            PerfilUsuarioSerializer, MedidaFormativaSerializer, SancionSerializer,
...
def test_views():
    ...
    try:
        from core.views import (
            ColegioViewSet, IncidentReportViewSet, TipoIncidenteViewSet,
            PerfilUsuarioViewSet, MedidaFormativaViewSet, SancionViewSet,
            ResolucionIncidenteViewSet,
            # Nuevas vistas
            ProtocoloProcesoViewSet, EtapaProtocoloViewSet,
            ProcesoIncidenteViewSet, EjecucionEtapaViewSet,
            ReglasAnonimatoViewSet, AccesoIdentidadViewSet
        )
...
def test_database_schema():
    ...
    try:
        from core.models import ProtocoloProceso, EtapaProtocolo
        
        # Verificar campos del modelo ProtocoloProceso
        protocolo_fields = [field.name for field in ProtocoloProceso._meta.fields]
        expected_fields = ['id', 'colegio', 'gravedad', 'nombre', 'descripcion', 
                          'plazo_total_dias', 'requiere_aprobacion_director', 
        ...
        expected_etapa_fields = ['id', 'protocolo', 'orden', 'nombre', 'descripcion',
                               'plazo_horas', 'es_plazo_habiles', 'responsable_rol',
                               'acciones_requeridas', 'documentos_requeridos',
                               'es_obligatoria', 'permite_anonimo', 'accion_si_anonimo',
                               'descripcion_anonimo']
