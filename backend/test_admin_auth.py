...
try:
    ...
    print(
        f"⚖️ Tipos legales: {TipoIncidente.objects.filter(es_categoria_legal=True).count()}"
    )
    print(
        f"🏫 Tipos personalizados: {TipoIncidente.objects.filter(es_categoria_legal=False).count()}"
    )
