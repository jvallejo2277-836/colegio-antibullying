...
def create_colegios():
    ...
    for data in colegios_data:
        colegio, created = Colegio.objects.get_or_create(rbd=data["rbd"], defaults=data)
...
def create_usuarios(colegios):
    ...
    for data in usuarios_data:
        ...
        user, created = CustomUser.objects.get_or_create(
            username=data["username"], defaults=user_data
        )
        ...
        perfil, created = PerfilUsuario.objects.get_or_create(
            user=user, defaults=perfil_data
        )
        perfiles_creados.append(perfil)
        print(
            f"Usuario: {user.username} ({perfil.tipo_usuario}) - {'Creado' if created else 'Ya existe'}"
        )
